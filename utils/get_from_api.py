...
def direct_network_input():
    ...
    """
    id_list = requests.get('http://127.0.0.1:5000/game_list?mode=network-input').json()
    row_list = []
    for id in id_list:
        detailed_game_data = requests.get('http://127.0.0.1:5000/api?data-source=network-input&game-id='+str(id)).json()
        row_list.append(detailed_game_data)
    input_df = pd.DataFrame(row_list)
    return input_df
...
def review_data(id=None, content=None):
    ...
    if id == None:
        ...
        comment_list = requests.get('http://127.0.0.1:5000/api?data-source=review-data-new&game-id='+str(id)+'&content=comment').json()
        lang_list = requests.get('http://127.0.0.1:5000/api?data-source=review-data-new&game-id='+str(id)+'&content=lang').json()
